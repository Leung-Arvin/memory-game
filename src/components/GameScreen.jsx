...
function GameScreen({boss, onRun}) {
  ...
  const startSequence = (ability, isDodge = false) => {
    ...
    const newSequence = newDots.sort(() => 0.5 - Math.random()).slice(0, numberOfDots);
  ...
  const enemyAttack = () => {
    ...
    setCurrentBossMove(boss.moves[Math.floor(Math.random() * boss.moves.length)]);
  ...
  return (
    <div className="game-container" onClick={returnToBossMenu}>
      ...
      <div className='health-bars-container'>
        <HealthBar health={playerhealth} maxHealth={boss.meowric_health} name="Meowric" />
      ...
      <div className='dialog-box'>
        ...
        {(gameState === 'enemy_attack' || dodgeSequence) && (
          <p>{boss.name} used {currentBossMove}</p>
        )}
        {gameState === 'running' && (
          <div className="dialog-box">
            <p>Got away safely!</p>
